...
echo "[TASK 0] Prerequisites - disable firewall"
...
apt install -qq -y kubelet=1.20.2-00 kubeadm=1.20.2-00 kubectl=1.20.2-00 >/dev/null 2>&1
...
then
  ...
  echo "[TASK 12] Adjusting configuration for master node"
  sed -e '/--port=0/ s/^#*/#/' -i /etc/kubernetes/manifests/kube-scheduler.yaml > /dev/null 2>&1
  sed -e '/--port=0/ s/^#*/#/' -i /etc/kubernetes/manifests/kube-controller-manager.yaml > /dev/null 2>&1
  systemctl restart kubelet.service >/dev/null 2>&1
...
then
  ...
  echo "[TASK 9] Post-installation tasks"
  echo -e "---------------------------------------\nManual intervention required, please execute following command on master node:\n"
  echo -e "kubectl label node <worker-name> node-role.kubernetes.io/worker=worker\n"
fi
